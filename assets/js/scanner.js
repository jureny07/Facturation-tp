...
function initScanner(videoId, resultId, inputId, onScan) {
  ...
  if (typeof Quagga === "undefined") {
    if (resultEl) {
      resultEl.innerHTML =
        '<span style="color:var(--red)">⚠ Erreur : QuaggaJS non chargé. Veuillez recharger la page.</span>';
    }
  ...
  function startScanner() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      if (resultEl)
        resultEl.innerHTML =
          '<span style="color:var(--red)">⚠ Votre navigateur ne supporte pas l\'accès caméra.</span>';
      running = false;
      return;
    }
    ...
    Quagga.init(
      ...
      function (err) {
        ...
        Quagga.start();
        if (resultEl)
          resultEl.innerHTML = '<span class="dot"></span> Scanner actif…';
      },
    );
    ...
    Quagga.onDetected(function (result) {
      ...
      if (resultEl) {
        resultEl.innerHTML =
          '<span class="dot"></span> Code détecté : <strong>' +
          code +
          "</strong>";
      }
  ...
  function stopScanner() {
    ...
    running = false;
    if (resultEl)
      resultEl.innerHTML = '<span class="dot"></span> Scanner arrêté.';
  }
